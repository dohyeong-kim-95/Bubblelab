...
```bash
mkdir sites/slop/my-idea
vim sites/slop/my-idea/index.html
...
```bash
git mv sites/slop/my-idea sites/games/my-idea && git push
...
전용 서브도메인을 갖는 토이도 가능: `sites/my-idea/` → `my-idea.bubblelab.dev`.

## 동작 방식

- Cloudflare Worker 하나(`worker/index.js`)가 모든 요청을 받아
  호스트명을 `dist/` 안의 폴더로 매핑합니다. (`slop.bubblelab.dev/x` → `dist/slop/x`)
- `scripts/build.mjs`가 `sites/` → `dist/`로 복사하면서, index.html이 없는
  카테고리 루트에 하위 폴더 목록 페이지와 404 페이지를 자동 생성합니다.
...
```bash
node scripts/build.mjs && npx wrangler dev
