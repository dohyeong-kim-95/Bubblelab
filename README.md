...
## 커밋 규칙 — 자기 서브도메인 밖은 `add` 하지 않습니다

**`git add -A`(`git add .`)를 쓰지 마세요.** 이 리포는 여러 사람·여러 에이전트
세션이 **같은 작업 트리를 동시에** 씁니다. 전체 추가는 남이 편집 중이던 파일까지
쓸어 담고, 반대로 내 변경도 남의 커밋에 실려 나갑니다.

```bash
git add invest/ _infra/invest.js      # ✅ 내가 만진 것만 이름으로
git add -A                            # ❌ 남의 미완성 작업까지 딸려 갑니다
```

- 커밋 전에 **`git status --short`로 남의 변경이 섞였는지 확인**하고, 내 것이
  아닌 파일은 그대로 둡니다.
