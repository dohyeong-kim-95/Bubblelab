...
> 개인 프로젝트입니다. 별도 명시가 없는 한 코드·에셋의 복제·재배포·상업적
> 이용을 허락하지 않습니다([`LICENSE`](./LICENSE)). 포함된 외부 오픈소스·에셋의
> 출처와 라이선스는 [`THIRD_PARTY_NOTICES.md`](./THIRD_PARTY_NOTICES.md)에
> 정리되어 있습니다.
...
**명시적인 요청이 없으면 자기 서브도메인 밖은 `add` 하지 않습니다.**

이 리포는 여러 사람·여러 에이전트 세션이 **같은 작업 트리를 동시에** 씁니다.
지금 `invest`를 고치고 있다면 커밋에 들어갈 것은 `invest/`와 그에 딸린 파일
(`_infra/invest.js`, `_src/invest-sink/`)뿐입니다. `duri/`나 `estate/`가 함께
바뀌어 있어도 그건 남이 하던 작업이니 그대로 둡니다.

```bash
git add invest/ _infra/invest.js      # ✅ 내 서브도메인과 그에 딸린 파일만
git add -A                            # ❌ 남의 미완성 작업까지 딸려 갑니다
```

다른 서브도메인까지 건드려야 한다면 **요청받았을 때만** 합니다. 공용 파일
(`_shared/`, `www/index.html`, 루트 문서)은 원래 여러 서브도메인이 함께 쓰므로
자기 작업에 필요한 범위까지는 포함해도 됩니다.

- 커밋 전에 **`git status --short`로 남의 변경이 섞였는지 확인**합니다.
- 작업 도중 `git log --oneline -1`이 예상과 다르면 그 사이 남이 커밋한 것입니다.
  내 변경이 이미 남의 커밋에 들어갔는지 `git log --oneline -- <내 파일>`로 봅니다.
- **이미 푸시된 이력은 혼자 판단해서 다시 쓰지 않습니다.** 알리고 함께 정합니다.

실제로 2026-08-11에 한 세션의 `invest/` 작업 전체가 다른 세션의 `git add -A`에
휩쓸려 `duri:` 커밋으로 푸시된 적이 있습니다. 배포는 멀쩡했지만 이력이 엉켜
되돌릴 수 없었습니다.

## 브랜치 규칙

- `main`이 배포 브랜치입니다. 모든 작업은 `main`에서 직접 합니다.
- 릴리스 트레인 브랜치(`next-week`)는 폐지했습니다. 하루에도 여러 번 배포하는
  리포에서 주간 단위 브랜치는 `main`을 따라오지 못한 채 뒤처지기만 했고,
  실제로 그 브랜치에만 남은 변경은 없었습니다. 배포 경계는 브랜치가 아니라
  Deploy 워크플로의 검사(테스트·빌드·아발론 산출물 동기화)가 담당합니다.
