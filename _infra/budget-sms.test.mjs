...
test("은행 출금·이체도 쓴 돈으로 담는다 — 금액에 '원' 이 없다", () => {
  /* 실기기에서 온 문자다. 원이 붙은 유일한 숫자가 잔액이라, 원만 보면 금액을 못 찾거나
   * 잔액을 담게 된다 — 출금·이체·송금 뒤에 붙은 숫자를 금액으로 본다. */
  const { entry } = read("[Web발신]\n<새마을금고>900330**4 도우너 출금100,000 잔액900,000원 08/23 08:46");
  assert.equal(entry.amount, 100000, "잔액 900,000 을 담으면 안 된다");
  assert.equal(entry.on, "2026-08-23");
  assert.equal(entry.memo, "새마을금고 도우너", "계좌 마스킹과 동사는 이름에서 걷어낸다");

  const moved = read("[Web발신] 국민은행 홍*동 이체 50,000 잔액 1,200,000원 08/23 10:00 김철수");
  assert.equal(moved.entry.amount, 50000);
  assert.equal(moved.entry.memo, "김철수", "이체는 받는 사람이 곧 단서다");
});

test("입금은 들어온 돈이라 담지 않는다", () => {
  assert.match(read("[Web발신] <새마을금고>900330**4 도우너 입금300,000 잔액1,200,000원 08/23").reason,
    /입금/);
});
