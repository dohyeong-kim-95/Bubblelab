...
/* 배포 시간은 실행마다 편차가 크다 — 같은 npm ci 가 2초일 때도 4분 15초일 때도 있었다
 * (총 13분 6초까지 간 적이 있고 그중 11분 16초가 두 번의 설치였다). 러너·레지스트리
 * 사정이라 줄일 수 있는 값이 아니므로 **설치에 걸리는 횟수와 노출 자체를** 줄였다.
 * 여기서 되돌아가지 않게 묶어 둔다 — 성능 회귀는 조용히 돌아오고 아무도 눈치채지 못한다. */
