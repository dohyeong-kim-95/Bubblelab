...
실제 시험 형식을 따른다: **대문제 2개**(각각 큰 데이터셋 1개 + 조건 필터 **중문제
3개**, 중문제당 3~7단계로 변수를 차례로 생성). `exam.html`이 `kind: "big"` 문제 중
2개를 무작위로 뽑는다. 시험 상태는 `bl-ds-exam-v1` 키 하나에 저장(시작/종료 시각,
문제, 통과 단계)되고, 풀이는 `solve.html?...&exam=1`로 연다.

대문제는 `problems.js`에서 `sections: [{title, cond, steps}]` 구조로 정의하며,
파일 끝의 평탄화 루프가 `steps`로 펼쳐 일반 문제와 같은 런타임을 쓴다(단계 id는
`a1/b3`처럼 중문제 접두사로 유일해야 함). 중문제 solution 체인도 순서대로 이어
실행되므로 섹션별 변수 이름(`df_a`, `df_b`…)이 겹치지 않게 짓는다. 대문제는 연습
목록("실전 대문제" 카테고리)에서도 풀 수 있다.

- 시험 중 `check.step()` 통과가 곧 제출 — `runtime.js`가 제한시간 안의 통과만
  시험 기록에 반영한다. 통과분은 시험 종료 시 연습 진행상황에도 합산된다.
- **엄격 모드**(시작 화면·대시보드에서 토글, `strict` 필드 + `_bl_set_strict`):
  채점 결과를 숨기고 "제출 접수"만 알린다(형식 오류는 알려줌). 단계 표시·제출
  카운트도 🔒로 가려지고, 종료 후 리포트에서 전부 공개된다. 엄격 모드 중에는
  연습 진행상황을 실시간 갱신하지 않는다(목록 화면으로 결과가 새는 것 방지).
