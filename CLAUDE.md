...
- main에 push하면 GitHub Actions가 자동 배포한다 (~1분). PR 불필요,
  main에서 직접 작업한다.
- **`git add -A`·`git add .` 금지 — 자기가 만진 파일만 이름으로 add한다.**
  여러 사람·여러 에이전트 세션이 같은 작업 트리를 동시에 쓴다. 전체 추가는 남의
  미완성 변경까지 쓸어 담고, 내 변경도 남의 커밋에 실려 나간다(실제로 그랬다).
  커밋 전 `git status --short`로 남의 것이 섞였는지 보고, 내 것이 아니면 그대로
  둔다. 이미 푸시된 이력은 혼자 다시 쓰지 말고 사용자에게 알린다.
