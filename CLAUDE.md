...
**배포 시간은 의존성 설치가 좌우한다.** 한때 13분 배포의 86%가 설치였다(루트 4분 15초 +
아발론 7분 01초, 실제 검증·빌드·배포는 다 합쳐 1분 35초). 그래서 두 가지를 걸어 뒀다 —
`node_modules` 를 통째로 캐시하고(`setup-node` 의 캐시는 내려받기만 아끼고 압축 해제·링크는
매번 다시 한다), **아발론은 소스·산출물의 내용 해시가 그대로면 검증을 건너뛴다**(몇 주씩
안 바뀌는데 매번 vite 를 새로 깔고 같은 결론을 냈다). 내용 해시라 한 글자만 바뀌어도
반드시 다시 돈다. `_infra/deploy-workflow.test.mjs` 가 이 둘과 wrangler 핀을 묶어 둔다 —
성능 회귀는 조용히 돌아오고 아무도 눈치채지 못하기 때문이다.
