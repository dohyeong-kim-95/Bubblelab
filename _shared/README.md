...
- 홈 버튼은 **자동 생성 홈을 가진 카테고리**(slop·util·games·assets)의 **카드
  ...
  가진 서비스에는 붙지 않습니다.
- 자리는 **우하단, 공유 버튼 바로 왼쪽**입니다(`body:has(#bl-share)`로 옆으로
  비킴). 둘 다 글자 없는 원형 아이콘(🏠 / 📤)이라 합쳐도 예전 "📤 공유" 하나와
  비슷한 폭(85px → 100px)에 머뭅니다.
- **상단은 쓰지 마세요.** 거의 모든 토이가 점수·타이머 HUD로 씁니다 — 실측에서
  상단 배치는 21개 중 7~9개에서 UI를 가렸고, 하단은 3개였습니다.
- **좌하단·하단 중앙도 쓰지 마세요.** 주간 기록 배지(`#bl-weekly`)는 폭과 높이가
  모두 가변입니다. 1~3위가 다 차면 390px 화면에서 높이 169px까지 자라고, 하단
  중앙 배치는 21개 중 13개 토이에서 겹쳤습니다. **위로 쌓는 것도 안전하지
  않습니다** — 배지가 3.8rem 높이를 넘어섭니다.
- 알려진 한계: 배지가 완전히 펼쳐지면 390px 이하 화면에서 우하단 아이콘 영역까지
  넘어옵니다. 이 버튼들이 생기기 전에도 배지는 `#bl-share`를 침범했으므로 새로
  생긴 문제는 아니지만, 근본 해결은 `#bl-weekly`의 `max-width`를 조이는 쪽입니다.
- **새 공용 고정 UI를 추가할 때는 네 모서리의 기존 점유를 먼저 확인하세요.**
  좌하단 `#bl-weekly`(가변)와 그 위 `#bl-claim`, 우하단 `#bl-home`·`#bl-share`와
  그 위 `#bl-suggest`가 이미 자리를 씁니다.
