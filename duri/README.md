...
- **일정 색**: 종일 일정은 색을 꽉 채운 알약 + 흰 글자, 시간 일정은 배경 없이
  ...
  4:1 안팎으로 확인).
- **여러 날 일정은 주(week) 단위로 하나의 막대**로 이어 그린다(구글 캘린더
  벤치마킹) — 같은 주 안에서는 제목이 한 번만 보이고, 주가 바뀌면(예: 설날
  연휴가 이번 주~다음 주에 걸칠 때) 그 주에서 또 한 번만 보인다. 겹치는 여러
  날 일정이 있으면 레인을 나눠 쌓는다(2인용이라 자주 겹치진 않아 단순한 그리디
  배치로 충분히 처리). 날짜 칸 안의 미리보기 목록에서는 여러 날 일정을 빼서
  중복 표시하지 않는다. 막대가 날짜 숫자보다 위로 올라오지 않도록, 한 주는
  [날짜 숫자 줄 → 막대 레인 → 일정 칩 줄] 순서로 그린다(`.cal-week-dates` →
