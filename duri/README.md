...
- **여러 날에 걸친 일정**: 종일 일정에서 "종료일"을 시작일보다 뒤로 잡으면 그
  범위의 모든 날짜에 나타난다(각 날짜에 같은 제목이 반복 표시되는 방식 — 격자
  칸을 가로지르는 띠는 아니다).
- **반복 일정**: 안 함/매일/매주/매월/매년 + 선택적 종료일(`until`). 회차마다
  따로 저장하지 않고 **시리즈 하나(원본 날짜 + 규칙)로 계산**해서 매번
  그려진다 — 그래서 **회차별 수정·삭제가 없다**: 편집하거나 삭제하면 시리즈
  전체가 바뀐다. 매월 반복은 시작일이 31일이면 31일이 없는 달은 자연히
  건너뛴다(예: 1/31 매월 → 2월엔 없고 3/31에 다시). **여러 날 종료일과 반복은
  함께 쓸 수 없다**(반복 설정 시 종료일 무시, 다이얼로그에 안내 문구).
