...
- `data/*.geojson`(시군구 386KB · 나라 577KB)만 브라우저 캐시를 받는다. 30일이라
  **주소에 내용 해시(`?v=`)를 달아** 둔다 — 안 그러면 파일을 고쳐 배포해도 폰은
  옛 것을 쓴다(`force-cache`). 세계 지도는 `_infra/duri-world-geo.mjs` 가 다시
  구울 때 `index.html` 의 해시까지 갈아 끼우고, 테스트가 어긋나면 거절한다. 게이트 뒤 사이트는 워커가
