...
export default {
  ...
  params: withSrc("mock", {
    tRCD: param({ family: "row", breaks: "센싱이 끝나기 전에 컬럼을 열어 읽는다 — 아직 갈라지지 않은 비트라인에서 값을 가져오므로 결과가 틀린다.", ns: 12, why: "행이 서기까지. 모바일은 속도보다 전력이 먼저라 데스크톱 쪽보다 여유가 있다." }),
    tRP: param({ family: "row", breaks: "비트라인이 아직 한쪽으로 치우친 채로 다음 행을 연다. 그 치우침이 새 전하공유를 눌러 이겨 엉뚱한 값으로 증폭된다.", ns: 12, why: "per-bank 프리차지. all-bank 는 이보다 길다." }),
    tRAS: param({ family: "row", breaks: "복원이 끝나기 전에 행을 닫는다 — 셀에 원래 전하가 덜 돌아간 채로 격리되어 **데이터가 손상된다.**", ns: 25, why: "복원이 끝날 때까지 행을 열어 둬야 하는 시간." }),
    tRC: param({ family: "row", breaks: "위의 tRAS·tRP 를 합친 것이라, 어기면 둘 중 하나를 어긴 것과 같다.", ns: 37, why: "같은 뱅크의 ACT → ACT. 정의상 tRAS + tRP 다." }),
    tRRD: param({ family: "spacing", breaks: "행 열기가 몰려 순간 전류가 튄다. 전압이 주저앉으면 진행 중이던 센싱까지 틀어진다.", ns: 6, why: "연속 ACT 간격. LPDDR5 는 _L/_S 를 나누지 않는다 — 뱅크그룹 구분이 규칙에 드러나지 않는다." }),
    tFAW: param({ family: "spacing", breaks: "짧은 창에 행을 다섯 번 열면 tRRD 를 지켰더라도 전원이 못 버틴다. 이 제약은 그 창을 따로 본다.", ns: 30, why: "어떤 창에서도 ACT 4번. 연속 ACT 간격(tRRD)만으로 낼 수 있는 것보다 길어야 뜻이 있다." }),
    tWR: param({ family: "turn", breaks: "센스앰프의 새 값이 셀에 다 써지기 전에 닫는다. **쓴 값이 사라진다** — 쓰기에서 가장 흔한 사고다.", ns: 18, why: "쓰기 데이터가 끝난 뒤 PRE 까지." }),
    tRTP: param({ family: "turn", breaks: "읽던 버스트가 끝나기 전에 행을 닫는다 — 나가던 데이터가 잘린다.", ns: 5, why: "READ 뒤 같은 뱅크 PRE 까지." }),
    tWTR: param({ family: "turn", breaks: "데이터 경로가 아직 쓰기 방향인데 읽기를 시작한다 — 버스가 충돌한다.", ns: 6, why: "쓰기에서 읽기로 방향을 돌리는 시간." }),
    tRFCab: param({ family: "refresh", breaks: "리프레시가 도는 중에 어레이를 건드린다 — 복원을 못 마친 행이 날아간다.", ns: 90, why: "all-bank 리프레시. 어레이 전체가 잠긴다." }),
    tRFCpb: param({ family: "refresh", breaks: "같은 이유다. 다만 그 뱅크만 해당한다.", ns: 45, why: "per-bank 리프레시. LPDDR 은 오래전부터 이걸 갖고 있었다." }),
    tREFI: param({ family: "refresh", breaks: "**이건 최소가 아니라 마감이다.** 넘기면 셀의 전하가 새어 나가 데이터가 조용히 썩는다 — 커맨드가 거절되는 게 아니라 값이 틀리기 시작한다.", ns: 2000, why: "리프레시 마감. 온도가 오르면 짧아진다." }),
    BL: param({ family: "latency", breaks: "버스트 길이는 지키고 말고 할 값이 아니라 설정이다. 다만 데이터가 DQ 를 몇 클럭 붙잡는지가 여기서 나온다.", ck: 16, why: "버스트 길이. LPDDR5 는 16/32 를 고른다. RL/WL 과 함께 \"데이터가 언제, 얼마나 오래 버스에 있는가\"를 정한다." }),
    RL: param({ family: "latency", why: "READ 레이턴시. 동작 모드마다 표가 갈라지는 것이 이 세대의 성격이라, 하나로 뭉뚱그린 모의값을 두지 않았다." }),
    WL: param({ family: "latency", why: "WRITE 레이턴시. 같은 이유로 비워 뒀다." }),
  }),
