 ...
 *
 * 파일을 이렇게 받는 이유: 파일 선택창은 어느 폴더에서 열릴지 페이지가 정할 수 없다.
 * 폴더를 찾아가는 대신 파일을 앱으로 보내는 쪽으로 뒤집으면 선택창이 통째로 없어진다
 * (폴더를 기억하는 길은 life/budget 이 File System Access API 로 따로 두고 있다).
 *
