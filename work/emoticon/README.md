...
- [`blink-research.md`](blink-research.md) — 우리 깜빡임이 시중 것과 다른 이유
  (속도·비대칭·최하점 체류). 타이밍을 설계할 때 읽는다.
- [`emotion-language.md`](emotion-language.md) — **감정을 무엇으로 보여줄 것인가.**
  얼굴·몸·효과 기호 세 채널, 만화 기호 어휘, 타이밍=감정, 카카오 구성 규칙
  (80% 일상 감정·중복 금지), duri 16종 커버리지 초안.
- [`pose-library.md`](pose-library.md) — **감정·동작별 포즈 문장 사전.**
  새 컷을 만들 때 여기서 가져다 쓴다. 실패한 표현도 같이 적혀 있다.
- [`nod-anatomy.md`](nod-anatomy.md) — 머리 회전(끄덕임·갸우뚱)의 해부·기하와
  작화 관례. **"이동"이 아니라 "부위 배치"로 쓰는 이유**가 여기 있다.
