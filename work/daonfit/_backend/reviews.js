...
// 이 파일은 다온핏 폴더가 통째로 인계될 때 함께 전달되는 리뷰·문의 동기화
// 백엔드 원본입니다. 리포의 _infra/reviews.js와 "완전히 독립된 동일 사본"이며
// 서로 import하지 않습니다 (work 폴더가 삭제돼도 리포가 깨지지 않고, 이 폴더만
// 넘겨도 그대로 구동됩니다).
//
...
//
// 네이버 커머스 API(판매자 인증)에서 리뷰와 상품 문의(Q&A)를 주기적으로 받아 DO에
// 캐시하고, 품목별 상세페이지가 이 캐시를 읽어 노출한다. 실제 API 호출부
// (fetchNaver*)는 판매자 자격증명이 있을 때만 실행되며, 없으면 mock으로 동작한다.
// 네이버에서 동기화된 항목은 source: "naver"를 달아 상세페이지에서 네이버 마크를
// 붙인다.
