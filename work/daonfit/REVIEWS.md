...
- **출처 표시**: 네이버에서 온 항목(`source: "naver"`)에는 초록색 **네이버 마크(N)**를,
  다온핏 자체 등록분(`source: "own"`)에는 `다온핏` 배지를 붙여 구분합니다. mock에는
  두 출처가 섞여 있습니다.
- **후기 작성**: 상세페이지의 "후기 남기기" 버튼으로 남긴 후기는
  `/_workreviews/daonfit/submit`(POST)로 저장되어 `submitted`(source: "own")로
  노출됩니다. 동기화분(`reviews:data`)과 분리 저장(`reviews:submitted`)되어
  cron·버전 갱신 시에도 보존됩니다.
