...
- 상세페이지(`goods/<상품>.html`)가 `reviews.js`로 `/_workreviews/daonfit`를
  호출 → 자기 상품(`window.blProduct`) 리뷰만 골라 렌더합니다.
- 서버(`_infra/reviews.js`)의 `fetchStoreReviews()`는 **판매자 자격증명이 없으면
  mock 리뷰를 반환**하고, 있으면 실제 커머스 API 경로(`fetchNaverCommerceReviews`)를
  탑니다.
- 동기화 결과는 `WorkReviewsDO`(프로젝트당 1개)에 캐시되고, 매일 cron(06:40 KST)이
  갱신합니다. 최초 조회 시에도 한 번 채웁니다.
- mock 리뷰 목록은 `_infra/reviews.js`의 `MOCK_REVIEWS`에 있습니다. 상품 slug는
  상세페이지 파일명과 일치합니다: `keybox`, `parking-keyring`, `vent-clip`,
...
2. **응답 스키마 검증** — `fetchNaverCommerceReviews()`가 사용하는 엔드포인트·필드명
   (`reviews`, `reviewId`, `reviewScore`, `reviewContent`, `createDate` 등)은
   실제 커머스 API 문서 기준으로 **재확인**하세요. API 버전에 따라 경로·필드가
   다를 수 있습니다.
