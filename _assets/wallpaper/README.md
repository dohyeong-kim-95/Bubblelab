...
- 미리보기 갤러리 — 아래 썸네일(원본 / 케이스 목업)을 누르거나 큰 그림을
  좌우로 밀어 넘깁니다.
- **내 기기에 맞게 잘라서 저장** — **기종을 고르면**(목록은 `assets/devices.js`,
  손으로 관리하며 없는 기종은 직접 입력) 그 해상도로 저장합니다. 잘라낼 상자는
  끌어서 위치를, 모서리를 끌어 크기를 정합니다(방향키·＋−·휠도 됨) — 가로세로비는
  기종 비율로 고정됩니다. 서버 왕복 없이 캔버스에서 처리하고, 계산은 등록 CLI와
  같은 `_shared/crop.js`를 씁니다.
  - 여기서는 **고른 기종 해상도로 그대로 저장합니다.** 상자를 줄여 당겨 자르면
    늘려서 채우고, 몇 배로 늘렸는지 화면에 알려줍니다(원본 최대 상자의 25%까지만
    줄일 수 있습니다). 등록 CLI의 "확대하지 않는다"는 카탈로그가 없는 해상도를
    광고하지 않기 위한 규칙이라 그대로 유지됩니다 — 방문자가 제 화면에 맞춰
    받아 가는 것과는 다른 상황입니다.
- **폰 케이스 목업** — 케이스 실루엣에 얹은 미리보기(갤러리 두 번째 장).
  카메라 섬이 왼쪽 위라 주인공이 왼쪽에 있는 그림은 가려지므로 **좌우 반전**
  토글이 있습니다(목업에만 적용되고 원본·잘라 저장에는 관여하지 않습니다).
  인쇄용 파일이 아닙니다.
