...
| `pullpin/` | 광고에서 본 그 게임 (핀 뽑기 구출 퍼즐) | 클리어 스테이지 최대 |
...
`pullpin`은 모바일 게임 가짜 광고의 원조인 핀 뽑기 퍼즐을 진짜로 만든 것입니다.
핀을 뽑아 보물을 냄비에 담고, 용암은 배수구로 흘려보내거나 건드리지 않고
남겨둡니다. 용암이 냄비에 들어가거나 보물이 모자라면 실패하며, 10개 스테이지
이후에는 필요 보물 수가 늘어난 채로 순환합니다.
